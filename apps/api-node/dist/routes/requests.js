...
export const registerRequestRoutes = async (app) => {
    ...
    app.patch("/api/onboardingRequests/:req_id", { preHandler: requirePermission(PERMISSIONS.SCHOOL_MANAGE) }, async (request, reply) => {
        ...
        if (body.status === "approved" && req.status !== "approved") {
            ...
            const school = await School.create({
                ...
                grades: req.grades,
                institution_type: req.institution_type ?? "school",
                is_active: true
            });
    ...
    app.patch("/api/userRequests/:req_id", { preHandler: requireRole(["admin", "platform_admin", "school_admin"]) }, async (request, reply) => {
        const params = z.object({ req_id: z.string() }).parse(request.params);
        const body = z.object({ status: z.string(), rejection_reason: z.string().optional() }).parse(request.body);
        const updated = await UserRequest.findOneAndUpdate({ id: params.req_id }, { $set: { ...body, approved_at: body.status === "approved" ? new Date() : null } }, { new: true });
        if (!updated)
            return reply.status(404).send({ detail: "Request not found" });
        return updated;
    });
