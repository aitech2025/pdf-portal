...
export const registerNotificationRoutes = async (app) => {
    ...
    app.post("/api/notifications/admin/send", { preHandler: requirePermission(PERMISSIONS.NOTIFICATION_SEND) }, async (request, reply) => {
        const body = z
            .object({
        ...
        let userQuery = { is_active: true, role: { $in: SCHOOL_ROLES } };
        ...
        const schoolIdsNeeded = [...new Set(recipients.map((u) => u.school_id).filter(Boolean))];
