...
export const registerPdfRoutes = async (app) => {
    ...
    app.get("/api/pdfs/:pdf_id", { preHandler: requireAuth }, async (request, reply) => {
        ...
        const [enriched] = await enrichPdfs([pdf.toObject()]);
        return enriched;
    });
    const streamPdf = async (request, reply, disposition, action) => {
        const params = z.object({ pdf_id: z.string() }).parse(request.params);
        const user = await requireCurrentUser(request, reply);
        if (!user)
            return;
    ...
    app.post("/api/pdfs/bulk-download", { preHandler: requireAuth }, async (request, reply) => {
        ...
        if (pdfs.length === 0)
            return reply.status(404).send({ detail: "No PDFs found" });
        ...
        if (denied.length) {
            for (const id of denied)
                await auditUnauthorized(user.id, "pdf_bulk_download", id, request);
    ...
    app.post("/api/pdfs/bulk-reassign", { preHandler: requirePermission(PERMISSIONS.PDF_UPLOAD) }, async (request, reply) => {
        ...
        if (body.subCategoryId) {
            ...
            if (!body.categoryId)
                body.categoryId = sub.category_id ?? undefined;
        }
        ...
        if (body.subCategoryId)
            update.sub_category_id = body.subCategoryId;
        if (body.status)
    ...
    app.delete("/api/pdfs/:pdf_id", { preHandler: requirePermission(PERMISSIONS.PDF_UPLOAD) }, async (request, reply) => {
        const params = z.object({ pdf_id: z.string() }).parse(request.params);
        const updated = await Pdf.findOneAndUpdate({ id: params.pdf_id }, { $set: { deleted_at: new Date(), is_active: false } }, { new: true });
        if (!updated)
            return reply.status(404).send({ detail: "PDF not found" });
        return { message: "PDF archived (soft deleted)" };
    });
    app.post("/api/pdfs/:pdf_id/restore", { preHandler: requirePermission(PERMISSIONS.PDF_UPLOAD) }, async (request, reply) => {
        const params = z.object({ pdf_id: z.string() }).parse(request.params);
        const updated = await Pdf.findOneAndUpdate({ id: params.pdf_id }, { $set: { deleted_at: null, is_active: true } }, { new: true });
        if (!updated)
            return reply.status(404).send({ detail: "PDF not found" });
        return serializeDoc(updated.toObject());
    });
