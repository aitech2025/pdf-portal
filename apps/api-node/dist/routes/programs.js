...
export const registerProgramRoutes = async (app) => {
    ...
    app.post("/api/programs", { preHandler: requirePermission(PERMISSIONS.CATEGORY_MANAGE) }, async (request, reply) => {
        const body = z
            .object({
        ...
        if (existing)
            return reply.status(409).send({ detail: "Program already exists" });
        return Program.create(body);
    });
    app.patch("/api/programs/:program_id", { preHandler: requirePermission(PERMISSIONS.CATEGORY_MANAGE) }, async (request, reply) => {
    ...
    app.delete("/api/programs/:program_id", { preHandler: requirePermission(PERMISSIONS.CATEGORY_MANAGE) }, async (request, reply) => {
        const params = z.object({ program_id: z.string() }).parse(request.params);
