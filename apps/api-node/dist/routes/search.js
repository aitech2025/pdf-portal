...
export const registerSearchRoutes = async (app) => {
    app.get("/api/search", { preHandler: requireAuth }, async (request, reply) => {
        ...
        return {
            query: query.q,
            pdfs: listResponse(pdfs.map((p) => serializeDoc(p)), pdfTotal),
            categories: listResponse(categories.map((c) => serializeDoc(c))),
            programs: listResponse(programs.map((p) => serializeDoc(p)))
        };
    });
};
