...
export const listResponse = (items: unknown[], total?: number) => ({
  items: Array.isArray(items) ? items.map((i) => (typeof i === "object" && i !== null ? serializeDoc(i as Record<string, unknown>) : i)) : items,
