...
export const registerFavoriteRoutes = async (app: FastifyInstance): Promise<void> => {
  app.get("/api/favorites", { preHandler: requireAuth }, async (request, reply) => {
    ...
    const items = rows
      .map((f) => {
        ...
        if (!pdf) return null;
        return { ...serializeDoc(f as Record<string, unknown>), pdf: serializeDoc(pdf as Record<string, unknown>) };
      })
      .filter(Boolean);
  ...
  app.post("/api/favorites", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!pdfId) return reply.status(400).send({ detail: "pdfId is required" });
    ...
    if (!allowed) return reply.status(403).send({ detail: "Insufficient permissions" });
    const existing = await Favorite.findOne({ user_id: user.id, pdf_id: pdfId });
    if (existing) return serializeDoc(existing.toObject() as Record<string, unknown>);
    const fav = await Favorite.create({ user_id: user.id, pdf_id: pdfId });
    return serializeDoc(fav.toObject() as Record<string, unknown>);
  });
  ...
  app.delete("/api/favorites/:pdf_id", { preHandler: requireAuth }, async (request, reply) => {
    ...
    if (!deleted) return reply.status(404).send({ detail: "Bookmark not found" });
