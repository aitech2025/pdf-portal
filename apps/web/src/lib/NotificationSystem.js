...
export const createNotification = async (recipientId, type, subject, message, method) => {
  try {
    await client.fetch('/notifications', 'POST', {
      recipientId,
      type,
      subject,
      message,
...
export const notifyOnboardingApproval = (recipientId, schoolId, password) =>
  createNotification(recipientId, 'onboarding_approval', 'Welcome to i-icon academy', `Your school has been approved. ID: ${schoolId}, Password: ${password}`, 'email');

export const notifyOnboardingRejection = (recipientId, reason) =>
  createNotification(recipientId, 'onboarding_rejection', 'Update on your i-icon academy Request', `Your request was not approved. Reason: ${reason}`, 'email');
